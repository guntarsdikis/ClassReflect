#!/bin/bash
# LLM Server Setup Script for Qwen2.5 7B with Secure API
set -e

echo "🚀 Setting up LLM API Server with Qwen2.5 7B..."

# Update system
sudo apt update && sudo apt upgrade -y

# Install required packages
sudo apt install -y python3 python3-pip python3-venv git htop nvidia-smi curl
...
echo "📦 Installing Qwen2.5 7B model..."
...
def load_config():
    ...
    config = {
        ...
        "users": {
            "admin": "$2b$12$example.hash.here",
            "demo": "$2b$12$example.hash.here"
        },
...
@app.get("/")
async def root():
    return {"message": "Qwen2.5 7B API Server", "status": "running", "auth": "required"}
...
async def api_info():
    return {
        ...
        "default_credentials": {
            "username": "demo",
            "password": "demo123",
            "api_keys": ["sk-demo123456789", "sk-test987654321"]
        }
    }
...
# Set permissions
sudo chown ubuntu:ubuntu -R /home/ubuntu/llm-api
...
echo ""
echo "🔍 Check status: sudo systemctl status llm-api"
