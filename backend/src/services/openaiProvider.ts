  ...
  async analyzeWithTemplate(
    ...
    const buildOpenAiCoachPrompt = () => {
      ...
      lines.push(
        ...
        '2. feedback (≥5 sentences, covering in order):',
        '   - one concrete strength observed',
        '   - one direct quote + timestamp as evidence',
        '   - description of the gap or missed opportunity',
        '   - why the gap matters for learning outcomes',
        '   - one growth step or coaching move',
      ...
      if (opts?.pauseMetrics) {
        ...
        lines.push('DATA TO USE:',
          ...
          '    "<criterion_name>": {',
          '      "score": number,',
          '      "feedback": "≥5 sentences covering strength, evidence, gap, why it matters, growth step",',
